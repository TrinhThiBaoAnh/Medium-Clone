import "./plans.css";
import { useContext, useState, useEffect } from "react";
import { Context } from "../../context/Context";
import axios from "axios";
import { useLocation } from "react-router";
import { PayPalScriptProvider, PayPalButtons } from "@paypal/react-paypal-js";
import React from "react";
export default function Plans() {
    const { user, dispatch } = useContext(Context);
    const PF = "http://localhost:5000/images/";
    const [show1, setShow1] = useState(false);
    const [show2, setShow2] = useState(false);
    const [approve, setAprrove]= useState(false);
    const createMember = async (e) => {
        e.preventDefault();
        let currentDate = new Date();
        try {
            const res = await axios.post("/members/", {
            user_id: user._id,
            start_date: new Date(),
            end_date: show1 ? new Date(currentDate.setMonth(currentDate.getMonth()+1)):new Date(currentDate.setMonth(currentDate.getMonth()+12)) ,
            fee: show1 ? 5 : 50,
          });
          console.log('create',res.data);
        } catch (err) {
            console.log('error', err);
        }
      };
    const updateUser = async (e) => {
            dispatch({ type: "UPDATE_START" });
            console.log('user',user);
            const updatedUser = {
                id: user._id,
                isMember: true,
            };
            try {
                const res = await axios.put("/users/" + user._id, updatedUser);
                console.log('update:', res.data);
                dispatch({ type: "UPDATE_SUCCESS", payload: res.data });
            } catch (err) {
                dispatch({ type: "UPDATE_FAILURE" });
            }
      };
  return (
   <div className="container-plans">
    <div className="img-plans">
        <img src="data:image/png;base64,"></img>
    </div>
    <div className="header-plans">
        <h2>Get unlimited access to everything on Medium</h2>
        <p>Plans starting at less than 1 $/week. Cancel anytime.</p>
        <ul>
            <li><i className="fa fa-check-square-o" aria-hidden="true"></i>No ads</li>
            <li><i className="fa fa-check-square-o" aria-hidden="true"></i>Support quality writing</li>
            <li><i className="fa fa-check-square-o" aria-hidden="true"></i>Access on any device</li>
        </ul>
    </div>
    <div className="body-plans">
        <div className="payment-container">
            <div className="monthly">
                <div className="monthly-above">
                    <b>Monthly</b>
                    <p>5 $ USD/month</p>
                    <br></br>
                </div>
                <div className="monthly-below">
                    <button class="button-select button1"  
                            onClick={(e) => {
                                setShow2(false);
                                setShow1(!show1);
                            }}>
                            Selected
                    </button>
                </div>
            </div>
            <div className="yearly">
                <div>
                    <b>Yearly</b>
                    <p>50 $ USD/month</p>
                    <p style={{color:"green"}}>(Save 10 $)</p>
                </div>
                <div>
                <button class="button-select button1" 
                        onClick={(e) => {
                                    setShow1(false);
                                    setShow2(!show2);
                                }}>
                        Selected
                </button>
                </div>
            </div>
        </div>
        <div>
            {show1 ? (
                <div className="payment-card">
                    <b>Total billed: 5 $ USD</b>
                    <p>By starting a Medium membership, you agree to our Membership Terms of Service. Your payment method will be charged a recurring 5 $ USD monthly fee, unless you decide to cancel. No refunds for memberships canceled between billing cycles.</p>
                    <input type="checkbox" id="check" name="check1" value="5"/>
                    <label htmlFor="check1">I agree with Medium's policy </label>
                    <br></br>
                    <button class="button button3" 
                                onClick={
                                    (e)=>{
                                       
                                        createMember(e);
                                        updateUser(e);
                                        setAprrove(true);
    
                                }}>Register
                    </button>
                </div>
            ) : null}
        </div>
        <div>
            {show2 ? (
                <div className="payment-card">
                <b>Total billed: 50 $ USD</b>
                <p>By starting a Medium membership, you agree to our Membership Terms of Service. Your payment method will be charged a recurring 50 $ USD yearly fee, unless you decide to cancel. No refunds for memberships canceled between billing cycles.</p>
                <input type="checkbox" id="check" name="check1" value="5"/>
                <label htmlFor="check1">I agree with Medium's policy </label>
                <br></br>
                <button class="button button3" 
                                onClick={
                                    (e)=>{
                                       
                                        createMember(e);
                                        updateUser(e);
                                        setAprrove(true);
                                       
                                       
                                    }}>Register
                </button>
            </div>
            ) : null}
        </div>
        {
            (user!==null) ? (
                <div>
                {(approve) ? (
                   <div className="payment-form">
                         <PayPalScriptProvider 
                                options={
                                    { 
                                    "client-id": 
                                    "AbL1aB3NQFD6Khbros-iYUhkF4Dgd5202BVR2cjQpzk2hd9QqaZkv1Nv9YFIKUZEUK3z_b8UkAHRoOqB",
                                    currency: "USD",
                                    intent: "capture",
                                    
                                    }
                                    }>
                            <PayPalButtons
                                createOrder={(data, actions) => {
                                    return actions.order.create({
                                        purchase_units: [
                                            {
                                                amount: {
                                                    value: show1 ? "5.00": "50.00",
                                                },
                                            },
                                        ],
                                    });
                                }}
                                onApprove={(data, actions) => {
                                    return actions.order.capture().then((details) => {
                                        const name = details.payer.name.surname + ' ' + details.payer.name.given_name;
                                        // setAprrove(true);
                                        updateUser();
                                        let currentDate = new Date();
                                        (show1 ? alert(`Your membership expired at ${new Date(currentDate.setMonth(currentDate.getMonth()+1))}`)
                                                : alert(`Your membership expired at ${new Date(currentDate.setMonth(currentDate.getMonth()+12))}`)
                                        )
                                    });
                                }}
                            />
                    </PayPalScriptProvider>
                    </div>) 
                        : null
                }
            </div>
            ):(
            <div>
                <br></br>
                <p style={{color: "red"}}>Please login to continue...</p>
            </div>
            )
        }
       
        
    </div>
   </div>
  );
}
